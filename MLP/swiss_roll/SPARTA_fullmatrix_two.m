...
tic
if flag == 1
    X = readmatrix("layer2-Xtrain-MLP-scaled.csv");
    X = transpose(X);
    X = X(2:(d+1),2:(T+1));

    Y = readmatrix("layer2-Ytrain-MLP-scaled.csv");
    Y = transpose(Y);
    Y = Y(2:(m+1),2:(T+1));
end
toc
...
save('results/W_fullp4_layer2', 'W');
save('results/Lambda_fullp4_layer2', 'Lambda');
save('results/estAp4_layer2', 'estA');
